...
import org.linkeddata.stack.limes.HttpSPARQLCreateGraph;
import org.linkeddata.stack.limes.HttpSPARQLUpdate;
import org.linkeddata.stack.limes.JsonResponse;
...
public class ImportRDF extends HttpServlet {
	...
	try {
		...
		message += "Imported " + inserted
			+ " accepted triples into the graph: " + acceptedGraph
			+ "\n";
	    }
		...
		message += "Imported " + inserted
			+ " triples for reviewing into the graph: "
			+ reviewGraph;
	    }
    ...
    // UpdateJDBC ujdbc = new UpdateJDBC(jdbcConnection, jdbcUser,
    // jdbcPassword);
    // ujdbc.loadLocalFile(file, graph);
    // }

    private static int queryImport(String destEndpoint, String graph,
	    String sourceEndpoint, String sparql) throws Exception {
    ...
    private static int httpUpdate(String endpoint, String graph, Model model)
	    throws Exception {
	...
	while (stmts.hasNext()) {
		...
		if (stmt.getSubject().isAnon()) {
		    String oldBN = stmt.getSubject().asNode()
			    .getBlankNodeLabel();
		...
		if (stmt.getObject().isAnon()) {
		    String oldBN = stmt.getObject().asNode()
			    .getBlankNodeLabel();
		    if (blancNodes.containsKey(oldBN)) {
    ...
    private static void httpCreate(String endpoint, String graph)
	    throws Exception {
	...
	if (!p.execute())
	    throw new Exception("Creating graph failed: " + queryString);
